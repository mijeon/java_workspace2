...
public class ProductDAO {
	...
	public int insert(Product product) {  //int subcategory_idx, String product_name, String brand, int price, String filename 매개변수 이용한 sql문
		Connection con=null;
	...
	public List selectAll() {   //뭔말이지
		...
		try {
			...
			while(rs.next()) {
				Product product=new Product();  //빈 product  //매번 new를 하는 이유? 테이블의 내용을 가지고 있는 rs에 대응하여 같은 수의 인스턴스를 생성하여 값을 넣어주기 위함
	...
	public Product select(int produce_idx) {
		...
		StringBuffer sb=new StringBuffer();
		sb.append("select t.topcategory_idx as topcategory_idx, topcategory_name, s.subcategory_idx as subcategory_idx, subcategory_name, product_idx, product_name, brand, price, filename");
